...
Note. The outline is intended to describe how an existing aspect has been 
changed and does NOT set out to describe new functionality.
...
The DLL tests may fail with a message to the effect that _Passed was expected 
but _Warning was returned. This is not a serious problem (yet) and can be ignored. 

Similarly, testing under different computer loads may invoke an occasional test
...
The initial development environment utilised Subversion (SVN) which provides 
atomic commits, revision control of folders and enables external repositories to 
be referenced via Externals. The combination is very powerful, particularly when 
TortoiseSVN is used as the Windows client.
