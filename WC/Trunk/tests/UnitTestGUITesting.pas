...
{$ELSE}
  ...
  GUITestRunner,
(*
If the following error message occurs here delete all the DCUs from the DCUs
folder and rebuild all projects. [Alt+P Alt+U]
[DCC Error] UnitTestGUITesting.pas(51):F2051 Unit GUITestRunner was compiled
 with a different version of TestFrameworkProxyIfaces.ITestProxy
*)
{$ENDIF}
  SysUtils,
  Classes;
...
type
  ...
  ISetUpTestCase = interface
  ['{27F75236-1504-443F-9B02-F698CB5436E4}']
    function SetUpCount: integer;
    function TearDownCount: integer;
  end;

  TSetupTestCase = class(TTestSetup, ISetUpTestCase)
  protected
    FSetupCount : integer;
...
begin
  ...
  TestFramework.ProjectRegisterTests('GUITest', [TFailuresTestCase.Suite]);
  FRunner.Suite := TestFrameworkProxy.RegisteredTests(TestFramework.Projects);
  Show;
...
begin
  TestFramework.ProjectRegisterTests('GUITest', [TSuccessTestCase.Suite]);
  FRunner.Suite := TestFrameworkProxy.RegisteredTests(TestFramework.Projects);
  Show;
...
begin
  ...
  if FRunner.ShowTestCasesWithWarningsAction.Checked then
    FRunner.ShowTestCasesWithWarningsAction.Checked := False;
...
begin
  TestFramework.ProjectRegisterTests('GUITest', [TTimeTestCase.Suite]);
  FRunner.Suite := TestFrameworkProxy.RegisteredTests(TestFramework.Projects);
  Show;
{$IFDEF DUNIT_CLX}
  FRunner.SetFocusedControl(FRunner);
...
//  SetAllowedLeakArray([288, 312]);
  TestFramework.ProjectRegisterTests('GUITest', [TStatusTestCase.Suite]);
  FRunner.Suite := TestFrameworkProxy.RegisteredTests(TestFramework.Projects);
  Show;
...
var
  LTestSuite : IRepeatedTest;
  SuccessTest : ITestCase;
begin
  SuccessTest := TSuccessTestCase.Suite;
  LTestSuite := TRepeatedTest.Suite( SuccessTest , 2);
  TestFramework.ProjectRegisterTests('GUITest', [LTestSuite]);
  FRunner.Suite := TestFrameworkProxy.RegisteredTests(TestFramework.Projects);
  Show;
...
//  CheckEquals(2, (FRunner.Suite.Tests[0] as ITestProxy).CountEnabledTestCases, 'Before Suite.Tests[0].Tests.Count');
  CheckEquals(4, FRunner.Suite.CountEnabledTestCases, 'Before CountEnabledTestCases');
  CheckEquals(4, TDunitDialogCracker(FRunner).Suite.CountEnabledTestCases, 'Before testcount');

//## Cant compare project and contained test   CheckSame(LTestSuite, FRunner.Suite, 'Before LTestSuite');
//## Cant compare project and contained test   CheckSame(SuccessTest, FRunner.Suite.Tests[0], 'Before SuccesTest');
...
//  CheckEquals(2, (FRunner.Suite.Tests[0] as ITestProxy).CountEnabledTestCases, 'After first Suite.Tests[0].Tests.Count');
//## Cant compare project and contained test   CheckSame(LTestSuite, FRunner.Suite, 'After first LTestSuite');
//## Cant compare project and contained test   CheckSame(SuccessTest, FRunner.Suite.Tests[0], 'After first SuccesTest');
  CheckEquals(4, FRunner.Suite.CountEnabledTestCases, 'After first CountEnabledTestCases');
  CheckEquals(4, TDunitDialogCracker(FRunner).Suite.CountEnabledTestCases, 'After first testcount');
...
var LTestDecSuite : ITestSuite;
    LTestSuite  : ITestCase;
    SuccessTest : ITestCase;
begin
  SuccessTest := TSuccessTestCase.Suite;
  LTestDecSuite := TSetupTestCase.Suite( SuccessTest);
  LTestSuite := LTestDecSuite;
...
//  CheckEquals(4, TDunitDialogCracker(FRunner).Suite.Tests.Count, 'Before testcount');

  CheckEquals(0, (LTestDecSuite as ISetUpTestCase).SetupCount, 'Before SetupCount');
  CheckEquals(0, (LTestDecSuite as ISetUpTestCase).TearDownCount, 'Before TearDownCount');
...
//  CheckEquals(4, TDunitDialogCracker(FRunner).Suite.Tests.Count, 'After first testcount');

  CheckEquals(1, (LTestDecSuite as ISetUpTestCase).SetupCount, 'After first SetupCount');
  CheckEquals(1, (LTestDecSuite as ISetUpTestCase).TearDownCount, 'After first TearDownCount');
...
//  CheckEquals(4, TDunitDialogCracker(FRunner).Suite.Tests.Count, 'testcount');

  CheckEquals(2, (LTestDecSuite as ISetUpTestCase).SetupCount, 'After second SetupCount');
...
//  CheckEquals(4, TDunitDialogCracker(FRunner).Suite.Tests.Count, 'Normal after testcount');
...
begin
  ...
  Check(FRunner.TestTree.Selected = FRunner.TestTree.Items[2], 'testing from non-test to next test node');
  FRunner.GoToNextSelectedTestAction.Execute;
  Check(FRunner.TestTree.Selected = FRunner.TestTree.Items[3], 'testing test node to next test node');
  FRunner.GoToNextSelectedTestAction.Execute;
  Check(FRunner.TestTree.Selected = FRunner.TestTree.Items[3], 'testing the end of the line, next should stay put');
  FRunner.GoToPrevSelectedTestAction.Execute;
  Check(FRunner.TestTree.Selected = FRunner.TestTree.Items[2], 'testing test to prev test');
  FRunner.GoToPrevSelectedTestAction.Execute;
  Check(FRunner.TestTree.Selected = FRunner.TestTree.Items[2], 'beg of the line, prev should stay put');
end;
{$ENDIF}
...
function TSetupTestCase.SetUpCount: integer;
begin
  Result := FSetupCount;
end;
...
function TSetupTestCase.TearDownCount: integer;
begin
  Result := FTearDownCount;
end;

initialization
  RefTestFramework.RegisterTests('', [T_TGUITestCase.Suite]);
  RefTestFramework.RegisterTests('GUI Tests', [TGUITestRunnerTests.Suite]);
end.
